...
#INICIALIZACION DE PYGAME, MIXER, Y PANTALLA_________________________________________________
    ...
    turnero1= pygame.image.load('./RECURSOS/turnero1.png')
    turnero2= pygame.image.load('./RECURSOS/turnero2.png')
    turnero3= pygame.image.load('./RECURSOS/turnero3.png')
    turnero4= pygame.image.load('./RECURSOS/turnero4.png')
...
#CONEXION CON EL SERVIDOR__________________________________
    ...
    s.send_string("Soy un cliente")
    respuesta = s.recv_string()
    setJugador(int(respuesta))
    jugador = int(respuesta)
    print "el jugador es: "+str(jugador)
...
#CAMBIO DE INTERFAZ SEGUN COLOR DEL JUGADOR________________________________________________
    ...
    #CONSULTA DE TURNO CON EL SERVIDOR__________________________________
    mensaje = "get turno"
    s.send_string(mensaje)
    respuesta = s.recv_string()
    modificarTurno(int(respuesta))
    print "el turno es: "+str(obtenerTurno())

    #_____________________________FIN DE CONSULTA
...
#GESTION DE EVENTOS_____________________________________________________________________
        ...
        #EVENTO CLICK DE MOUSE__________________________________________________
            if jugador == turnito:
                ...
                print "El turno es: ", turnito
                #CONTROL DE MENSAJES RECIBIDOS DESDE SERVIDOR___________________________
                if retraso == 1:#se espera a que se muestre el parques en pantalla, luego si no es turno de este cliente, estara atento a recibir informacion
                    s.send_string("nada")
                    respuesta0 = s.recv_string()

                    print "se recibe del servidor lo siguiente:"
                    print respuesta0
                    print "________________\n"

                    if respuesta0 == "actualizar ficha":#peticion que notifica de movimientos hechos en otros tableros
                        s.send_string("listo")
                        ...
                        #aux = [f.id, f.casillaa, f.casillaas, f.pasos, npos[0], npos[1], jugador]

                        print "se recibe del servidor la siguiente lista:"
                        print idficha,casillaficha,casillasficha,pasosficha,posxficha,posyficha,player
                        print "________________\n"
                        for f in Fichas:#actualizacion de la ficha
                            if f.id == int(idficha):
                                f.casillaa = int(casillaficha)
                                f.casillaas = int(casillasficha)
                                f.pasos = int(pasosficha)
                                f.mover([int(posxficha),int(posyficha)])

                        #Fichas.update()
                        #Fichas.draw(pantalla)
                    if respuesta0 == "turno finito":#peticion que notifica fin de turno de jugador actual en el servidor
                        s.send_string("listo")
                        respuesta2 = s.recv_string()
                        print "se recibe del servidor lo siguiente:\nTurno:"
                        print respuesta2
                        print "________________\n"
                        modificarTurno(int(respuesta2))

                    if respuesta0 == "terminar":
                        s.send_string("terminando")
                        algo = s.recv_string()
                        fin = True
                        s.close()
                #_____________________FIN DE CONTROL DE MENSAJES
...
#GESTION DEL JUEGO___________________________________________________________________
        ...
        if salida[jugador] == 1:
                        ...
                        if f.casillaa == pu.casilla:
                            if encontrado == 0:
                                ...
                                if aux == [0,0]:
                                    ...
                                    f.pasos = 5
                                    salida[jugador] = 5
                                    #envio de informacion de movimiento a servidor_______________
                                    aux2 = [f.id, f.casillaa, 55, f.pasos, aux[0], aux[1], jugador]

                                    s.send_string("registrar movimientos")
                                    algo = s.recv_string()

                                    s.send_string(str(f.id))
                                    something = s.recv_string()
                                    s.send_string(str(f.casillaa))
                                    something = s.recv_string()
                                    s.send_string(str(55))
                                    something = s.recv_string()
                                    s.send_string(str(f.pasos))
                                    something = s.recv_string()
                                    s.send_string(str(aux[0]))
                                    something = s.recv_string()
                                    s.send_string(str(aux[1]))
                                    something = s.recv_string()
                                    s.send_string(str(jugador))
                                    algo = s.recv_string()
                                    #______________fin de envio de informacion
            ...
            movee = 0
            cassel = 0
            #seccion de informacion de turnos_________________
            s.send_string("turno finito")
            turnorecibido = s.recv_string()

            print "Modificando Turno a "+turnorecibido
            modificarTurno(int(turnorecibido))
            retrasar = True
            #_______________________-fin seccion informacion turno
                            ...
                            if f.id == jugador*4:
                                if casi == 1000:
                                    ...
                                    if npos == [0,0]:
                                        ...
                                        #envio de informacion de movimiento a servidor_______________
                                        aux3 = [f.id, f.casillaa, f.casillaas, f.pasos, npos[0], npos[1], jugador]
                                        s.send_string("registrar movimientos")
                                        algo = s.recv_string()
...
#GESTION DE PANTALLA___________________________________________________________________
        ...
        pantalla.blit(turnando,[750,16])
        pantalla.blit(cuadro,[820,350])
        pantalla.blit(cuadro,[820,425])
        pantalla.blit(cuadro,[820,500])
        pantalla.blit(cuadro,[820,575])
