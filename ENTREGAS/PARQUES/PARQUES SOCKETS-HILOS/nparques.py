...
#GESTION DE EVENTOS_____________________________________________________________________
        ...
        #EVENTO CLICK DE MOUSE__________________________________________________
            if jugador == turnito:
                ...
                if retraso == 1:#se espeara a que se muestre el parques en pantalla, luego si no es turno de este cliente, estara atento a recibir informacion
                    mi_socket.send("Proceso Escucha".encode("ascii"))
                    respuesta0 = mi_socket.recv(1000)
                    auxiliar = respuesta0.decode("ascii")
                    respuesta0 = auxiliar
                    if respuesta0 == "turno finito":#peticion que notifica fin de turno de jugador actual en el servidor
                        mi_socket.send("listo".encode("ascii"))
                        respuesta2 = mi_socket.recv(1000)
                        auxiliar = respuesta2.decode("ascii")
                        respuesta2 = auxiliar
                    ...
                    if respuesta0 == "actualizar ficha":#peticion que notifica de movimientos hechos en otros tableros
                        mi_socket.send("listo".encode("ascii"))
                        respuesta3 = ""
                        respuesta3 = mi_socket.recv(1000)
                        auxiliar = respuesta3.decode("ascii")
                        respuesta3 = stringlista(auxiliar) #aux = [f.id, f.casillaa, f.casillaas, f.pasos, npos[0], npos[1], jugador]
                        for f in Fichas:#actualizacion de la ficha
                            if f.id == respuesta3[0]:
                                f.casillaa = respuesta3[1]
                                f.casillaas = respuesta3[2]
                                f.pasos = respuesta3[3]
                                f.mover([respuesta3[4],respuesta3[5]])
                    if respuesta0 == "no cambio":
                        respuesta0 = ""
...
#GESTION DEL JUEGO___________________________________________________________________
        ...
        if salida[jugador] == 1:
                        ...
                        if f.casillaa == pu.casilla:
                            if encontrado == 0:
                                ...
                                if aux == [0,0]:
                                    ...
                                    mensaje = "registrar movimientos"
                                    mi_socket.send(mensaje.encode("ascii"))
                                    algo = mi_socket.recv(1000)
                                    mensaje = "salida"
                                    mi_socket.send(mensaje.encode("ascii"))
                                    algo = mi_socket.recv(1000)
                                    fichasalir = listastring(aux2)
                                    mi_socket.send(fichasalir.encode("ascii"))
                                    algo = mi_socket.recv(1000)
            ...
            mensaje = "turno finito"
            mi_socket.send(mensaje.encode("ascii"))
            algo = mi_socket.recv(1000)
            mensaje = "listo"
            mi_socket.send(mensaje.encode("ascii"))

            turnorecibido = mi_socket.recv(1000)
                            ...
                            if f.id == jugador*4:
                                if casi == 1000:
                                    ...
                                    if npos == [0,0]:
                                        ...
                                        mi_socket.send(mensaje.encode("ascii"))
                                        algo = mi_socket.recv(1000)
                                        fichaenviar = listastring(aux3)
                                        mi_socket.send(fichaenviar.encode("ascii"))
                                        algo = mi_socket.recv(1000)
                                        #______________fin de envio de informacion
